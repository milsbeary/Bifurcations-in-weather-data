#THIS SCRIPT CONVERTS CLEAN SINGLE VARIABLE DATA INTO TIME SERIES FOR TEMPRATURE 3 HOUR GAPS

#Last modified by M. H. Kent Dec 25, 2023


#Delete previous information stosource('~/Desktop/TIME_SERIES_GENERATOR_TMP_3HR_GAP_600.R', chdir = TRUE)red 
rm(list=ls(all=T))
...
#tmp_path <- "/Users/milsbeary/Desktop/clean_tmp_dep"
tmp_path <- "/home/mhkent/scratch/clean_tmp_dep"
...
L = 600 #Specifying the time series length 
gap = 1 #Specifying the spaceing in hours
...
df_ub <- data.frame(data.table::fread(tmp_files[q])) #Import dataframe
#df_ub <- data_in[1:2000,]
#print(nrow(df_ub))

#df_ub <- data.frame(data.table::fread(tmp_files[29])) 
#summary(df_ub)
#nrow(df_ub)


#single_variable_generator <- function(df_ub, L, gap) { #If we want to define the prosses as a function 
...
THS_PRAM   <- colnames(df)[7] #Selects the parameter we are using 
THT_TIMS   <- th_df_window[1:L, 7] #Extracts the unbroken temprature time series

TH_APND <- c(as.numeric(THS_NAME), THS_REPT, THS_LATI, THS_LONG, as.character(THS_STAR), as.character(THS_ENDT), THS_PRAM, THT_TIMS) #Creates a vector to append  to "Thour_Timeseries"
#print(TH_APND)
...
write.csv(Thour_Timeseries_Final, paste("/home/mhkent/scratch/clean_timeseries/clean_tmp_ts_1/1hour_ts_", unique(df$STATION), rep_types[m], ".csv", sep = "")) #Writes a csv for the specified stations
#write.csv(Thour_Timeseries_Final, paste("/Users/milsbeary/Desktop/clean_tmp_dep", unique(df$STATION), rep_types[m],  ".csv", sep = "")) #Writes a csv for the specified stations
